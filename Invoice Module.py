...
query10 = 'select distinct a.PolicyNo, b.PolicyName, a.FromDate, a.ToDate, a.ClassName\
        ...
        where a.ToDate = (select max(ToDate) from tblclassmaster aa where aa.PolicyNo = a.PolicyNo)'
...
def generate_periods(policy_start):
        # Convert policy_start_date to a pandas Timestamp
        start_date = pd.Timestamp(policy_start)

        #add 1 year to create the policy end date
        end_date = start_date + pd.DateOffset(years=1) - pd.DateOffset(days=1)
        #return the next policy year end date in the format 'Month Year'
        # next_policy_end = end_date.strftime('%B %Y')

        if payment_plan == 'Quarterly':
            quarters = [
            (start_date, start_date + pd.DateOffset(months=3) - pd.DateOffset(days=1)),
            (start_date + pd.DateOffset(months=3), start_date + pd.DateOffset(months=6) - pd.DateOffset(days=1)),
            (start_date + pd.DateOffset(months=6), start_date + pd.DateOffset(months=9) - pd.DateOffset(days=1)),
            (start_date + pd.DateOffset(months=9), end_date)
        ]
            if payment_period == '1st Quarter':
                period = f'{quarters[0][0].strftime("%B %Y")} - {quarters[0][1].strftime("%B %Y")}'
            elif payment_period == '2nd Quarter':
                period = f'{quarters[1][0].strftime("%B %Y")} - {quarters[1][1].strftime("%B %Y")}'
            elif payment_period == '3rd Quarter':
                period = f'{quarters[2][0].strftime("%B %Y")} - {quarters[2][1].strftime("%B %Y")}'
            elif payment_period == '4th Quarter':
                period = f'{quarters[3][0].strftime("%B %Y")} - {quarters[3][1].strftime("%B %Y")}'
            else:
                raise ValueError("Invalid payment period for Quarterly plan.")
        elif payment_plan == 'Bi-Annual':
            halves = [
            (start_date, start_date + pd.DateOffset(months=6) - pd.DateOffset(days=1)),
            (start_date + pd.DateOffset(months=6), end_date)
            ]
            if payment_period == '1st Half':
                period = f'{halves[0][0].strftime("%B %Y")} - {halves[0][1].strftime("%B %Y")}'
            elif payment_period == '2nd Half':
                period = f'{halves[1][0].strftime("%B %Y")} - {halves[1][1].strftime("%B %Y")}'
            else:
                raise ValueError("Invalid payment period for Bi-Annual plan.")
        elif payment_plan == 'Tri-Annual':
            thirds = [
            (start_date, start_date + pd.DateOffset(months=4) - pd.DateOffset(days=1)),
            (start_date + pd.DateOffset(months=4), start_date + pd.DateOffset(months=8) - pd.DateOffset(days=1)),
            (start_date + pd.DateOffset(months=8), end_date)
            ]
            if payment_period == '1st Third':
                period = f'{thirds[0][0].strftime("%B %Y")} - {thirds[0][1].strftime("%B %Y")}'
            elif payment_period == '2nd Third':
                period = f'{thirds[1][0].strftime("%B %Y")} - {thirds[1][1].strftime("%B %Y")}'
            elif payment_period == '3rd Third':
                period = f'{thirds[2][0].strftime("%B %Y")} - {thirds[2][1].strftime("%B %Y")}'
            else:
                raise ValueError("Invalid payment period for Tri-Annual plan.")
...
def generate_input_fields(client, plan):
        ...
        for i, plan in enumerate(select_plan):
                ...
                periods = generate_periods(policy_period)
                ...
                if i_num_lives is not None and i_num_lives > 0:
                        if payment_plan == 'Annual':
                              ...
                              plan_renewal.append({'PlanID':policyno + '-' + str(i+1), 'client':client, 'Plan': plan, 'Type': 'INDIVIDUAL', 'Count': i_num_lives, 'Annual Rate (#)':i_premium, 'Period': periods, 'InvPeriod': payment_period, 'Bi-Annual Amount (#)': i_num_lives*(i_premium/2)})
                        elif payment_plan == 'Tri-Annual':
                              plan_renewal.append({'PlanID':policyno + '-' + str(i+1), 'client':client, 'Plan': plan, 'Type': 'INDIVIDUAL', 'Count': i_num_lives, 'Annual Rate (#)':i_premium, 'Period': periods, 'InvPeriod': payment_period, 'Tri-Annual Amount (#)': i_num_lives*(i_premium/3)})   
                        elif payment_plan == 'Quarterly':
                              plan_renewal.append({'PlanID':policyno + '-' + str(i+1), 'client':client, 'Plan': plan, 'Type': 'INDIVIDUAL', 'Count': i_num_lives, 'Annual Rate (#)':i_premium, 'Period': periods, 'InvPeriod': payment_period, 'Quarterly Amount (#)': i_num_lives*(i_premium/4)})
                if f_num_lives is not None and f_num_lives > 0:
                        if payment_plan == 'Annual':
                              ...
                              plan_renewal.append({'PlanID':policyno + '-' + str(i+1), 'client':client, 'Plan': plan, 'Type': 'FAMILY', 'Count': f_num_lives, 'Annual Rate (#)':f_premium, 'Period': periods, 'InvPeriod': payment_period, 'Bi-Annual Amount (#)': f_num_lives*(f_premium/2)})
                        elif payment_plan == 'Tri-Annual':
                              plan_renewal.append({'PlanID':policyno + '-' + str(i+1), 'client':client, 'Plan': plan, 'Type': 'FAMILY', 'Count': f_num_lives, 'Annual Rate (#)':f_premium, 'Period': periods, 'InvPeriod': payment_period, 'Tri-Annual Amount (#)': f_num_lives*(f_premium/3)})
                        elif payment_plan == 'Quarterly':
                              plan_renewal.append({'PlanID':policyno + '-' + str(i+1), 'client':client, 'Plan': plan, 'Type': 'FAMILY', 'Count': f_num_lives, 'Annual Rate (#)':f_premium, 'Period': periods, 'InvPeriod': payment_period, 'Quarterly Amount (#)': f_num_lives*(f_premium/4)})
...
def generate_invoice(inv_data, table_data, payment_plan):
    ...
    if payment_plan == 'Annual':
        ...
        headers = ['Plan', 'Type', 'Count', 'Annual Rate (#)', 'Bi-Annual Amount (#)']
    elif payment_plan == 'Tri-Annual':
        headers = ['Plan', 'Type', 'Count', 'Annual Rate (#)', 'Tri-Annual Amount (#)']
    elif payment_plan == 'Quarterly':
        headers = ['Plan', 'Type', 'Count', 'Annual Rate (#)', 'Quarterly Amount (#)']
    else:
        raise ValueError("Invalid payment plan provided.")
    ...
    table2 = doc.tables[1]
    table2.cell(0, 0).text = f'{generate_periods(policy_period)} Premium Invoice'
...
if client is not None and select_plan is not None:
    ...
    inv_data = {
            ...
            'InvPeriod': generate_periods(policy_period)
    }
            ...
            try:
                    for plan in renewal_data:
                            cursor.execute('INSERT INTO tbl_renewal_portal_invoice_module_plan_data\
                                            ...
                                            plan['Annual Rate (#)'] if payment_plan == 'Annual' else (plan['Annual Rate (#)'] / 2 if payment_plan == 'Bi-Annual' else plan['Annual Rate (#)'] / 3 if payment_plan == 'Tri-Annual' else plan['Annual Rate (#)'] / 4),
                    ...
                    blob_client.upload_blob(invoice.getvalue(), overwrite=True)
